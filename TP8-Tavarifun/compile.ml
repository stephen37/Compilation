...
let rec compile_expr env nxt_var e =
  match e.expr with
    ...
    | Egetref (e) ->
      ...
      (* let some y = x suppose que x est une ref NE pas stoker dans le tas la valeur de x
      mais la valeur stocké à X*)

      (*Compile e1 Compile e2 Stocker val à addr Renvoie unit*)
    | Esetref (e1, e2) ->
    ...
    | Estruct params ->
        ...
        in
        let shift =
            match Env.find tid env with
            | Structure d -> d
            | _ -> failwith "ERROR"
        in
        let nbParams = List.length shift in

        let struct_code = List.fold_left2(fun struct_code (f, fexp) (id2, dec) ->
            if String.compare f id2 == 0 then
                struct_code
                @@ push v1
                @@ (compile_expr env nxt_var fexp)
                @@ pop v0
                @@ pop v1
                @@ sw v0 areg (dec, v1)
            else
    ...
    | Eget (e, id) ->
        ...
        let shift =
            ...
            | _ -> failwith "ERROR"
        in
        let (_, dec) = List.find(fun (id2, _) -> String.compare id2 id == 0) shift in
        (compile_expr env nxt_var e)
        @@ pop v1
        @@ lw v0 areg (dec, v1)
        @@ push v0
...
let rec compile_instr_list env nxt_global il =
  match il with
    ...
    | Istruct (id, params) :: il ->
        let shift =
            let rec rec_shift l i =
                match l with
                | [] -> []
                | (e, _)::s -> (e, i) :: (rec_shift s (i + data_size))
            in rec_shift params data_size
        in
        let newEnv = Env.add id (Structure shift) env in
        compile_instr_list newEnv nxt_global il
    | _ -> not_implemented()
