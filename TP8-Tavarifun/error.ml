...
let to_string e =
  match e with
    ...
    | Not_a_reference t -> sprintf "this expression has type %s, it's not a reference" (typ_to_string t)
    | Not_a_value -> sprintf "this expression is not a value"
    | Type_not_found -> sprintf "there is no type with those fields"
